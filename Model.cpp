...
std::vector<float> Model::getFloats(json accessor)
{
    ...
    for (unsigned int i = beginningOfData; i < beginningOfData + lengthOfData; i += 4)
    {
        ...
        float value; memcpy(&value, bytes, sizeof(float));
...
std::vector<GLuint> Model::getIndices(json accessor)
{
    ...
    if (componentType == 5125)
    {
        ...
        {
            unsigned char bytes[] = { data[i], data[i + 1], data[i + 2], data[i + 3] };
            unsigned int value; memcpy(&value, bytes, sizeof(unsigned int));
    ...
    else if (componentType == 5123)
    {
        for (unsigned int i = beginningOfData; i < byteOffset + accByteOffset + count * 2; i += 2)
        {
            ...
            unsigned short value; memcpy(&value, bytes, sizeof(unsigned short));
            indices.push_back((GLuint)value);
        }
    }
    else if (componentType == 5122)
    {
        ...
        {
            unsigned char bytes[] = { data[i], data[i + 1] };
            short value; memcpy(&value, bytes, sizeof(short));
            indices.push_back((GLuint)value);
        }
    }
    return indices;
...
std::vector<Texture> Model::getTextures()
{
    ...
    for (unsigned int i = 0; i < JSON["images"].size(); i++)
    {
        ...
        if (!skip)
        {
            if (texPath.find("baseColor") != std::string::npos || texPath.find("_B") != std::string::npos)
            {
            ...
            else if (texPath.find("metallicRoughness") != std::string::npos || texPath.find("_ORM") != std::string::npos)
            {
...
std::vector<glm::vec2> Model::groupFloatsVec2(std::vector<float> floatVec)
{
    ...
    {
        glm::vec2 v;
        for (unsigned int j = 0; j < floatsPerVector; j++) v[j] = floatVec[i + j];
        vectors.push_back(v);
    }
    return vectors;
}
